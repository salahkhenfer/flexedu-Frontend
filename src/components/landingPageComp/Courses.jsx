...
const Courses = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const response = await axios.get("http://localhost:3000/Geust/Courses");
            ...
            ? courses.map((course) => (
                <SwiperSlide key={course?.id}>
                  <CourseCard
                    ...
                    lessonsCount={course?.Course_Videos.length} // Optional: the number of lessons based on Course_Videos
                  />
                </SwiperSlide>
              ))
            : null}
