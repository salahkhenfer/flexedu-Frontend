      ...
      {rejected && (
        <div className="bg-red-50 border-l-4 border-red-500 p-4 mb-8 rounded-md">
          <div className="flex items-center">
            ...
            <div>
              <h3 className="text-red-800 font-semibold">Payment Rejected</h3>
      ...
      <div className="mb-12">
        ...
        <div className="bg-white rounded-xl p-6 shadow-md">
          ...
          <div className="space-y-2 text-gray-700">
            <p>Please send the screenshot of the payment via email:</p>
            {PAYMENT_EMAIL && (
              <p className="font-medium">Email: {PAYMENT_EMAIL}</p>
            )}
            {PAYMENT_CCP && <p className="font-medium">CCP: {PAYMENT_CCP}</p>}
            {PAYMENT_CCP_CLE && (
              <p className="font-medium">Clé: {PAYMENT_CCP_CLE}</p>
            )}
            {PAYMENT_CCP_NAME && (
              <p className="font-medium">CCP Name: {PAYMENT_CCP_NAME}</p>
            )}
            {PAYMENT_EMAIL_TITLE && (
              <p className="font-medium">
                Use this title in the email: {PAYMENT_EMAIL_TITLE}
              </p>
            )}
            {PPYMENT_RIP && <p className="font-medium">RIP: {PPYMENT_RIP}</p>}
          </div>
        </div>
      </div>
        ...
        <Formik
          ...
          validate={(values) => {
            ...
            if (!ccpNumber) {
              ...
              errors.CCP_number = "CCP number must be in 'xxxxx/xx' format";
            }
              ...
              try {
                if (!image_state) {
                  ...
                  if (Image_Response.status === 200) {
                    ...
                    Navigate(`/Student/Summaries/${summary.id}`);
              ...
              <div className="mb-6">
                <div className="flex  justify-center items-center">
                  <input
                    ...
                    className="hidden"
                    onChange={(event) => {
                      setimage_state(event.currentTarget.files[0]);
                    }}
                    ref={fileInputRef}
                    className="hidden"
                  />
